...
import { auth, firestore, storage } from "../IIb/config";
...
const Header = () => {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full bg-black border-b-[2px] z-50"style={{"border":"white"}}>
        <div className="p-2 max-w-6xl mx-auto  flex items-center justify-between "style={{"color":"yellow"}}>
          <div>
            <Link to="/">
              <img src="/images/logo-full.png"  className="h-9 w-auto object-cover"  alt="" />
              </Link>
          ...
          <div>
            <div className="hidden sm:block p-3 rounded-lg bg-yellow-100 items-center" style={{"color":"yellow"}}>
              <form onSubmit={(e) => e.preventDefault()}>
                <div className="flex gap-3 text-yellow-700">
                  ...
                  <input
                    type="text"
                    placeholder="Search"
                    className="bg-transparent h-full outline-none"style={{"color":"black"}}
                  />
                </div>
              </form>
            </div>
          ...
          {user ? (
            <>
              <div className="flex items-center text-slate-800 text-2xl gap-1"style={{"color":"orange"}}>
                <NavLink to="/">
                  <HomeIcon />
                </NavLink>
                <div>
                ...
                {user ? (
                  ...
                  >
                    <img
                      className="h-7 md:border-[1px] border-slate-900 rounded-full aspect-square"
                      // eslint-disable-next-line react/jsx-no-duplicate-props
                      src={ "https://cdn-icons-png.flaticon.com/512/3135/3135768.png" } src={user?.photoURL}  alt={user?.name}     />
                    {menuOpen && (
                      <div className="absolute z-30 -bottom-5 w-[200px] right-0 translate-y-full bg-black shadow rounded text-xs" style={{"color":"yellow"}}>
                        <ul className="flex flex-col p-3 justify-center gap-2">
                          <li onClick={() => setMenuOpen(!menuOpen)}>
                            <Link
                              to={`/${user?.username}`}
                              className="flex items-center gap-1"style={{"color":"yellow"}}
                            >
                              <div>
                                <ProfileIcon />
                              </div>
            ...
            <div className="flex gap-3 items-center"style={{"color":"yellow"}}>
              <Link
                to="/login"
      ...
      {modelOpen && (
        <div className="fixed top-0 flex items-center justify-center left-0 w-screen h-screen z-50"style={{"color":"yellow"}}>
          <div
          ...
          <div className="p-4 rounded-lg overflow-hidden">
            <div className="bg-white rounded-lg">
              <div className="border-b-3 py-2 text-center" style={{"fontFamily":"Import","color":"black"}}>Create Post</div>
              <div className="max-w-[522px] w-full h-full aspect-square flex items-center justify-center">
                <div className="flex flex-col w-full overflow-hidden items-center justify-between gap-5">
                  {images ? (
                    <>
                      {images.length === 1 ? (
                        <img
                  ...
                  <div className="flex justify-center w-full">
                    <div className="mb-3 w-full px-2">
                      ...
                      <form onSubmit={uploadImage}>
                        ...
                        {images && (
                          <>
                            <input type="text" onChange={(e) => setCaption(e.target.value)}    className="text-black p-2 border-2 mb-3 outline-none w-full "  placeholder="Add a caption"   value={caption}  />                  
