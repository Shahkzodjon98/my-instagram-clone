...
// firebase
import {  arrayRemove,  arrayUnion,  collection,  doc,  getDoc,  onSnapshot,  query,  setDoc,  where,} from "firebase/firestore";
import { firestore, storage } from "../IIb/config";
  ...
  return (
    <div>
      ...
      <div className="mt-1 min-h-screen">
        {profileUser && (
          <main className="bg-gray-100 bg-opacity-35 md:h-[940px]">
            <div className="lg:max-w-5xl lg:mx-auto mb-8">
              <header className="flex flex-wrap items-center p-6 md:py-20">
                <div className="md:w-3/12 md:ml-36">

                  <div className="relative group w-40 h-50 md:w-40 md:h-40 object-cover overflow-hidden rounded-full">
                    {profileUser?.id === user?.uid && (
   <div className="absolute cursor-pointer opacity-0 group-hover:opacity-100 duration-75 transition-all top-0 left-0 h-full w-full bg-black/70 flex items-center justify-center text-align-center text-3xl md:text-5xl text-white aspect-square">                        <EditProfileIcon
                          htmlFor="profile-image"
                          onClick={() => profilePic.current.click()}/>
          
                        <input type="file"name="profile-image"  className="hidden h-full w-full"
                         id="profile-image"            
                         onChange={(e) => {    
